...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos):
        ...
        # inference mode: with kv-cache 
            ...
            # 将键和值标记嵌入存储到它们各自的缓存中 (kv_cache)  
            self.cache_k[:bsz, start_pos:start_pos + seq_len] = xk  
            self.cache_v[:bsz, start_pos:start_pos + seq_len] = xv    
            keys = self.cache_k[:bsz, :start_pos + seq_len]  
            values = self.cache_v[:bsz, :start_pos + seq_len]    
            keys = Attention.repeat_kv(keys, self.n_rep)  # keys[bsz,seq_len,n_heads,head_dim]
            values = Attention.repeat_kv(values, self.n_rep)  # values[bsz,seq_len,n_heads,head_dim]  
