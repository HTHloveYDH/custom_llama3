...
def call_model(model, messages, max_tokens, is_final_answer=False):
...
"""},
    ...
    start_time = time.time()
    final_data = call_model(messages, 1200, is_final_answer=True, custom_client=custom_client)
    end_time = time.time()
    thinking_time = end_time - start_time
    total_thinking_time += thinking_time
