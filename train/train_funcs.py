...
def train_on_epoch(model, data_loader, optimizer, device:str, steps_per_epoch:int, grad_accum_steps:int, \
...
@torch.no_grad()  
def valid_on_epoch(model, raw_model, data_loader, device:str, val_steps:int, epoch:int, dp:bool, \
                   master_process:bool):
    model.eval()  
    val_loss_tracker = []
    device_type = get_device_type(device)
