...
def main():
    ...
    for epoch in range(epochs):
        ...
        train_on_epoch(
            modules if pp > 1 else model, pp_schedule, train_data_loader, optimizer, device, 
            steps_per_epoch, grad_accum_steps, epoch, log_interval, parallel_args, master_process
        )
        # validate current weights on validation dataset shard of current process
        valid_on_epoch(
            modules if pp > 1 else model, pp_schedule, val_data_loader, device, val_steps, 
            epoch, parallel_args, master_process, lora
        )
